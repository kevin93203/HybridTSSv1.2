...
void TupleSpaceSearch::DeleteRule(const Rule& rule) {
    ...
    /*std::vector<Rule>::iterator it = std::find(rules.begin(),rules.end(),rule);
...
void PriorityTupleSpaceSearch::DeleteRule(const Rule& rule) {
    /*if (i < 0 || i >= rules.size()) {
        printf("Warning index delete rule out of bound: do nothing here\n");
        printf("%lu vs. size: %lu", i, rules.size());
        return;
